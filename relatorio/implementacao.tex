...
implementação em MATLAB dos algoritmos do gradiente normalizado e \textit{least-square} normalizado.
...
As condições iniciais do sistema são os valores para $y(0)$, $\theta_0$ e $P_0$, este último somente no caso do método \textit{least-squares} normalizado.
...
O sinal de referência deve possuir pelo menos seis frequências para esse sistema, pois há seis parâmetros desconhecidos (Seção 3.5 - Tao). Sendo assim, foi escolhido um sinal de referência com três senóides de frequências distintas. Além disso, o sistema é dito com excitação persistente se $u(t)$ não possui frequências do regressor $\phi(t)$. 
%
...
O vetor de parâmetros ideais é definido como:
%
\begin{gather}
\theta^* =
\begin{bmatrix}
...
As equações do filtro $\frac{1}{\Lambda(s)}$ podem ser descritas como um sistema no espaço de estados:
%
\begin{gather*}
\omega_1 = 
\begin{bmatrix}
